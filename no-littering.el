...
(defvar no-littering-tmp-directory
  (expand-file-name (convert-standard-filename "tmp/") user-emacs-directory)
  "The directory where packages place their temporary data files.
This variable has to be set before `no-littering' is loaded.")
...
(defun no-littering-expand-tmp-file-name (file)
  "Expand filename FILE relative to `no-littering-tmp-directory'."
  (expand-file-name (convert-standard-filename file)
                    no-littering-tmp-directory))
...
(cl-letf (((symbol-function 'etc)
          ...
          ((symbol-function 'tmp)
           (symbol-function #'no-littering-expand-tmp-file-name)))
...
;;; Built-in packages
    ...
    (setq ede-project-placeholder-cache-file (tmp "ede-projects.el"))
    (eval-after-load 'erc
      `(make-directory ,(var "erc/dcc/") t))
    (setq erc-dcc-get-default-directory    (var "erc/dcc/"))
    (setq erc-log-channels-directory       (tmp "erc/log-channels/"))
    ...
    (setq filesets-menu-cache-file         (tmp "filesets-menu-cache.el"))
    ...
    (setq image-dired-temp-image-file      (tmp "image-dired/temp-image"))
    (setq image-dired-temp-rotate-image-file (tmp "image-dired/temp-rotate-image"))
    (setq kkc-init-file-name               (var "kkc-init.el"))
    (eval-after-load 'newsticker
      `(make-directory ,(var "newsticker/") t))
    (setq newsticker-cache-filename        (tmp "newsticker-cache.el"))
    ...
    (setq rcirc-log-directory              (tmp "rcirc-log/"))
    (setq recentf-save-file                (tmp "recentf-save.el"))
    ...
    (setq url-cache-directory              (tmp "url/"))
...
;;; Third-party packages
    ...
    (setq async-byte-compile-log-file      (tmp "async-bytecomp.log"))
    ...
    (setq elmo-split-log-file              (tmp "elmo-split-log"))
...
directories."
    ...
    (setq geben-temporary-file-directory   (tmp "geben/"))
    ...
    (setq helm-github-stars-cache-file     (tmp "helm/github-stars-cache.el"))
    (setq helm-net-curl-log-file           (tmp "helm/helm-curl.log"))
    ...
    (setq jabber-avatar-cache-directory    (tmp "jabber/avatar-cache"))
    (eval-after-load 'jabber
      `(make-directory ,(tmp "jabber/avatar-cache/") t))
    ...
    (setq logview-cache-filename           (tmp "logview-cache"))
    ...
    (setq lsp-session-file                 (tmp "lsp/session.el"))
    ...
    (setq magithub-cache-file              (tmp "magithub-cache.el"))
    ...
    (setq omnisharp-cache-directory        (tmp "omnisharp/cache"))
    ...
    (setq org-journal-cache-file           (tmp "org/journal-cache.el"))
    (setq org-recent-headings-save-file    (tmp "org/recent-headings.el"))
    ...
    (setq pyim-dcache-directory            (tmp "pyim/dcache/"))
    ...
    (eval-after-load 'sx
      `(make-directory ,(tmp "sx/") t))
    (setq sx-cache-directory               (tmp "sx/"))
    ...
    (setq treemacs-last-error-persist-file (tmp "treemacs/persist-last-error.org"))
    ...
    (setq wl-temporary-file-directory      (tmp "wanderlust"))
    (setq x86-lookup-cache-directory       (tmp "x86-lookup/"))
    (eval-after-load 'xkcd
      `(make-directory ,(tmp "xkcd/") t))
    (setq xkcd-cache-dir                   (tmp "xkcd/"))
